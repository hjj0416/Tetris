...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        /// item刷新
        /// </summary>
        public class RefreshItemtEvent : UnityEvent<int, Transform> { }
        public readonly RefreshItemtEvent OnRefreshItem = new RefreshItemtEvent();
        ...
        [SerializeField]
        private ScrollRectEvent m_OnVelocityChanged = new ScrollRectEvent();
        public ScrollRectEvent onVelocityChanged { get { return m_OnVelocityChanged; } set { m_OnVelocityChanged = value; } }
        ...
        public Vector2 velocity
        {
            ...
            {
                m_Velocity = value;
                m_OnVelocityChanged.Invoke(m_Velocity);
            }
        }
        ...
        private void createPool(GameObject template)
        {
            ...
            //create
            UISimplePool pool = new UISimplePool("LoopScrollRect", _poolRoot, template);
        ...
        private void InitItemSize()
        {
            if (sameSize && prefab!=null)
            {
                ...
                if (layout != null)
                {
                    ...
                    //Debug.LogError("--------  size : " + itemWidth + " " + itemHeight);
                }
            }
        }
        ...
        {
            return itemTypeEnd >= totalCount-1;
        }
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
            ...
            ContentOverflow = sizeFilled > sizeToFill;
        }
        ...
        public void RefillCellsAndStay()
        {
            if (!Application.isPlaying || prefab == null)
                return;
            itemTypeEnd = 0;
        ...
        //public void AppendCells(int offset = 0)
        //{
        //    //根据是否铺满来决定要不要加offset
        //    if (IsFull())
        //    {
        //        //Debug.LogError("----------  scroll is full");
        //        itemTypeEnd = itemTypeStart + offset;
        ...
        protected float NewItemAtStart()
        {
            ...
            {
                itemTypeStart--;
                Transform newItem = InstantiateNextItem(itemTypeStart);
                newItem.SetAsFirstSibling();
                size = Mathf.Max(GetSize(newItem.transform as RectTransform), size);
            }
        ...
        protected float NewItemAtEnd()
        {
            ...
            {
                Transform newItem = InstantiateNextItem(itemTypeEnd);
                size = Mathf.Max(GetSize(newItem.transform as RectTransform), size);
                itemTypeEnd++;
                if (totalCount >= 0 && itemTypeEnd >= totalCount)
                {
        ...
        private Transform InstantiateNextItem(int itemIdx)
        {            
            Transform nextItem = spawn();
